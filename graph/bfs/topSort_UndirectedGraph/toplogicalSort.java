...
//一般是用在有向图中，因此统计入度的时候，入度为0的就丢进queue中；
//one task has to be done before the other one, res(task ordering)
//BFS方法做topological sort……大概就是
1)建图，算入度, maintain queue(indegree=0 node)
2)每次移除入度为0的点直到无点可移除(expansion)……一起被移除的点等级都一样…
/**
