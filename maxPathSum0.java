...
public int maxPathSum0(TreeNode root){
  int globalMax = Intger.MIN_VALUE;
  maxPath(root, 0, globalMax);
  return globalMax;
}
private void maxPath(TreeNode root, int prefixSum, int globalMax){
  ...
  if(root.left == null && root.right == null){
    globalMax = Math.max(prefixSum + root.value, globalMax);
    return;
  }
  //step1
  maxPath(root.left, prefixSum+root.value, globalMax);
  maxPath(root.right, prefixSum+root.value, globalMax);
}
