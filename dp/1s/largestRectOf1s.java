...
// step1. 从上到下的direction找连续最长1存入一个新的matrix里
// step2. 遍历这个新的matrix，把每个点当作长方形的右下顶点，看能组成多大面积的长方形，这里我用了两个方法，
...
//不断算出最大面积来更新，这个方法的时间复杂度是O(n*m^2) [n is the row of the matrix, m is the column of the matrix],
    ...
    // helper functions
     // top[i][j] means how many column wise consecutive 1s
      private int[][] lookUp(int[][] matrix, int row, int col) {
